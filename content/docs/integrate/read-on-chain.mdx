...
export async function checkVerification(
  ...
  const lastVerifiedAt = Number(account.data.readBigInt64LE(48));
  const score = account.data.readUInt16LE(60);
  const age = Math.floor(Date.now() / 1000) - lastVerifiedAt;

  return { verified: score >= minScore && age <= maxAgeSeconds, score };
}
```
...
pub fn do_something(
    ...
    let last_verified = i64::from_le_bytes(data[48..56].try_into().unwrap());
    ...
    require!(
        now.saturating_sub(last_verified) <= max_age_secs,
...
def read_anchor(client: Client, wallet: Pubkey) -> tuple[int, int] | None:
    """Returns (trust_score, last_verified_at) or None if never verified."""
    pda, _ = Pubkey.find_program_address([b"identity", bytes(wallet)], ENTROS_ANCHOR)
    info = client.get_account_info(pda).value
    if info is None:
        return None
    last_verified_at = int.from_bytes(info.data[48:56], "little", signed=True)
    trust_score = int.from_bytes(info.data[60:62], "little")
    return trust_score, last_verified_at
```
...
The read patterns above cover scoring, display, and gating. Read both fields: `trust_score` for how consistently a wallet has verified, and `last_verification_timestamp` for how long ago it last did.

For a claim, a vote, or anything else worth the friction, have the client run a verification at the point of the action so the Anchor you read was written moments earlier. Writes go through the SDK.
