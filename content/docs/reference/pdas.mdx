...
The `entros:human-operator` metadata entry holds JSON-encoded UTF-8 inside the registry's Borsh `Vec<u8>` envelope, with the shape `{ anchorPda, trustScore, verifiedAt, wallet }`. Any agent owner can write it, and it grants no permission. Read it with `readAgentOperatorSnapshot` as a [historical snapshot](/docs/integrate/agent-anchor).
