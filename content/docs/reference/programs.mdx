...
Verifies Groth16 proofs of the Hamming-distance circuit. Its `verify_proof` instruction runs in the same transaction as `mint_anchor` or `update_anchor` and writes a `VerificationResult` PDA, which the Anchor program then reads through an account constraint bound to this program's ID. Neither program invokes the other.
...
| `set_validator_pubkey` | Authority-only write of the validator pubkey used for the mint-receipt binding (entros-anchor 6016–6022). Reallocs the protocol config to the post-binding layout the first time it runs. |
| `migrate_admin` | Emergency administrative migration of the protocol authority |
| `register_validator` | Stake to join the Anonymity Ring (activates as the ring decentralizes) |
...
| Field | Type | Purpose |
| --- | --- | --- |
| `min_stake` | `u64` | Minimum validator stake (SOL on devnet today; transitions to $ENTROS as the ring decentralizes) |
