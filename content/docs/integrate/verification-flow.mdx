...
This is the Tier 1 path, and the right choice for around 90% of integrations. It is also the only one that answers whether the person is there right now, which is why it belongs on a claim, a vote, or anything else worth the friction. Tier 2 (custom UX with the Pulse SDK directly) is documented at [Reference → SDK](/docs/reference/sdk).
...
Use `walletPubkey` to identify the user. Use `trustScore` to ask for more than "verified once", for example a 100 floor on airdrop eligibility. Use `attestationPda` and `txSig` for audit trails.

The callback fires on a verification that just happened, so the score arrives against someone present. Gating on the callback rather than on a later read is what makes that true.
