...
Each verification captures roughly twelve seconds of three involuntary behavioral signals on the user's device—voice prosody, hand tremor, and touch dynamics. Those signals are reduced on-device to a 308-number statistical summary, hashed into a 256-bit fingerprint, and proven against the previous fingerprint via a zero-knowledge circuit. The proof, the new commitment, and a small protocol fee land on Solana. Nothing else.
...
- **Not a standing claim.** An Anchor records that a human verified, and how consistently. Whether that human is present right now is what a verification at the point of the action establishes, which is why an application gating something valuable asks for one. See [Trust Score: what to gate on](/docs/concepts/trust-score#what-to-gate-on).
