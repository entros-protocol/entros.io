...
description: Drop in <EntrosVerify /> to run a wallet-connected Entros verification.
---

The verification flow ships as `@entros/verify`. The React component opens an entros.io popup, runs the capture, signs the wallet transaction, and returns a result through `onVerified`.

This is the Tier 1 path for applications that want Entros to host the wallet and capture flow. Applications can request it before a high-value action. Tier 2 custom UX is documented in the [SDK reference](/docs/reference/sdk).
...
Use `walletPubkey` to identify the wallet. Use `trustScore` and recency to apply your policy. Use `txSig` to inspect the confirmed verification transaction.

`attestationPda` is a deterministic address. SAS issuance occurs after the verification transaction and can fail without failing verification. Fetch that account before treating the attestation as issued.
