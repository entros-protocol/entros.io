...
For an action worth real friction, neither read is the whole answer. Run a verification at the point of the action and read the Anchor after it, so the number you gate on describes someone who is present. See [Trust Score: what to gate on](/docs/concepts/trust-score#what-to-gate-on).
...
The executor currently writes only `wallet-connected`. The field carries both values because walletless is a defined tier that is not currently offered.
