...
`walletPubkey` identifies the user. `trustScore` lets you gate higher-stakes actions, for example a 100 floor on airdrop eligibility. It arrives from a verification that just completed, so it describes someone present. `attestationPda` and `txSig` give you audit trails.
