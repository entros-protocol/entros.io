...
The protocol publishes the SDK as `@entros/pulse-sdk` on npm. Programs are deployed in place, so program IDs do not change between versions. Material changes to instructions, PDAs, or scoring functions are documented here.

## Pulse SDK

### 4.x

The 4.x line canonicalises capture at 16 kHz and reworks the transport and failure taxonomy.

**4.12.0** Adds Agent Operator Permit support. `readAgentState` reads an agent's Metaplex Core owner and registry agent wallet at one slot. The agent wallet binding builds the registry's Ed25519 check and `set_agent_wallet` instructions. `findLatestVerificationTransaction` finds a wallet's newest qualifying transaction. `readAgentOperatorSnapshot` reads the old `entros:human-operator` entry as a historical snapshot, and the Agent Anchor functions become deprecated aliases. Anchors on older account layouts now decode in browser bundles that define no global `Buffer`.

**4.11.0** Prepares and submits request-bound proofs for configured deployments. Proof inputs are validated before proving, and artifact retention is bounded.

**4.10.0 to 4.10.2** Adds confirmed identity and transaction evidence for application policies, with typed attestation availability. The evidence reader samples a live clock after network responses and allows one bounded catch-up for issuer clock skew.

**4.9.2** First-verification minting derives Token-2022 associated token accounts inside the SDK. The package no longer depends on `@solana/spl-token`.

**4.9.1** Upload progress handling now survives native retry resets and host callback errors. SimHash rejects non-finite feature values before projection.

**4.9.0** Adds the inactive projection 2 client contract. Requests bind compatibility evidence and wallet authorization to the issued challenge.

**4.8.0** Adds compact proof and Anchor instructions while keeping the legacy instruction layouts available.

**4.7.1** Baseline reset requests now send explicit reset intent.

**4.7.0** Adds versioned feature projections, purpose-bound validator receipts, and projection-aware baseline migration.
