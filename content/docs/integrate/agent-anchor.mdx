...
For a registration flow that acts on the answer now, read the operator's Anchor PDA as well and check `last_verification_timestamp` against your own window. Registering a new agent is also a natural place to ask the operator for a verification, which makes the score you read describe someone present.

This pattern turns "anyone can register an agent" into "any verified human with a sustained track record can register an agent." It does not stop a determined operator from registering many agents. Combined with rate limits or per-operator caps, it stops the cheap-to-mass-register ones.
